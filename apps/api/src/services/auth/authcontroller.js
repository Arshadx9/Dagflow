...
export const registrationcontrol = async (req, res, next) => {
    try {
        ...
        res.status(201).json(Responseformatter.success("registered successfully ", {
            apikey: result.api
        }));
    }
    catch (error) {
...
export const logincontrol = async (req, res, next) => {
    try {
        ...
        res.status(200).json(Responseformatter.success("logged in successfully", {
            user: result.user
        }));
    }
    catch (error) {
...
export const onboardingcontrol = async (req, res, next) => {
    try {
        if (!req.userID)
